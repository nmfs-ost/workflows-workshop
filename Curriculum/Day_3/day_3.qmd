...
This is the code you would run to export a landings table, but **it is not yet functional.**
```{r}
stockplotr::table_landings(
  dat = petrale,
  group = "fleet",
...
::: {.callout-warning}
We are working on finishing the exporting capability of `table_landings()`. When it's functional, the landings table will be exported using the `make_rda` and `tables_dir` arguments, in parallel to `plot_biomass()` shown above.
:::
...
First, let's copy our example table rda into a "tables" folder:

```{r}
dir.create(file.path(getwd(), "tables"))
file.copy(from = file.path(getwd(), "example_plots", "ex_table.rda"),
          to = file.path(getwd(), "tables"))
```

Then, add the table to the {asar} tables doc:
