...
Welcome! Please open the [Day 3 communal notes doc](https://docs.google.com/document/d/1nVlmmZgp8vCHZKCMHnBm43bf-mtk7pSA1t2FLHEoyZA/edit?tab=t.vffehz79fm7y#heading=h.lwix6dbx3lr8) and participate in the icebreaker exercise.
...
-   [Day 3 communal notes doc](https://docs.google.com/document/d/1nVlmmZgp8vCHZKCMHnBm43bf-mtk7pSA1t2FLHEoyZA/edit?tab=t.vffehz79fm7y#heading=h.lwix6dbx3lr8)
...
If you have already converted your output: great! Import it into your R environment so that you can handle it as an object like "petrale". For example:

```{r eval=FALSE}
load("petrale_conout.rda")
petrale <- out_new
```
...
petrale <- stockplotr::convert_output(output_file,
                                      save_dir = file.path(getwd(), "petrale_conout.rda")
                                      )
```
...
# Make your figure
your_figure <- ggplot2::ggplot(petrale2,
...
**Shortcut for NEFSC**: Run this code to automatically add a row to your alt text/captions csv with sample data for the Executive Summary's ssb figure (label = "fig-ssb"):

```{r eval=FALSE}
# Make small data frame with ssb figure's information
new_row <- data.frame(
  col1 = "ssb", # identical to the chunk label ("fig-ssb") MINUS "fig-"
  col2 = "figure",
  col3 = "my caption",
  col4 = "my exec summary figure alt text",
