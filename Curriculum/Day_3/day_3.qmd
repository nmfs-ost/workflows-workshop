...
We are working closely with Quarto developers to incorporate the accessibility featrues found in `asar` into the base Quarto software. This means that, in the future, you may not need to use `asar::add_tagging()` and `asar::add_alttext()` to add accessibility features to your report. Instead, you may be able to add these features directly within your .qmd files. With that said, we will make sure we maintain compatibility of reports throughout quarto updates.
