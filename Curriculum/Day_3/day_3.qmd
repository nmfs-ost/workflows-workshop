...
`asar` should already be installed if you attended day 2, but if not, please install it first:
...
| biomass-at-age | biomass, landings, and catch time series |
...
![](images/day3_ex_plt4.png){fig-alt="why doesn't this work."}
...
<!-- Add brief reorientation of template files/structure -->
...
![](images/petrale_table.png){width=50% fig-alt="Example of a table produced with the 'Direct coding-in-qmd table workflow' option for adding tables to `asar` reports."}
...
![](images/petrale_figure.png){width=50% fig-alt="Example of a figure produced with the 'Direct coding-in-qmd figure workflow' option for adding figures to `asar` reports."}
...
<!-- Add note here: if no alt tex provided, then it'll take the image path as alt text. We'll fix that in next step -->
...
<!-- Take out/edit the following text for direct image-based workflow images ?? need more testing to see if alt text is added -->

You must add alternative text ("alt text") for images in a separate csv if you added figures using either of these workflows:

* the [rda-based workflow](#sec-rda-based-fig)
* the [direct coding-in-qmd workflow](#sec-code-in-qmd-fig)

This step is *not* essential if you added *all of your figures* using the [direct image-based workflow](#sec-image-based-fig).
...
Now your PDF's figures have alt text! If you hover your cursor over an image, you should see the alt text pop up, like this:

![](images/alt_text_fig.png){fig-alt="Screenshot of an example PDF-based, tagged report with alternative text. A cursor hovering over a biomass figure shows its alternative text in a bubble."}

::: {.callout-note appearance="minimal"}
questions, then 10 minute break
:::

# Demo for a realistic, customized asar report with stockplotr plots

<!-- Not sure we'll have time for this. If so, update bullets below -->
...
-Add the landings figure .png file, located in the 'example_plots' folder, to your figures doc.
