...
<!-- Sam reviewed several ways to make asar reports more reproducible, efficient, with markdown- no pointing/clicking necessary to bold text, etc. -->
<!-- The container where you'll be editing your documents, with markdown in them, is Quarto. -->
<!-- Quarto is a publishing system for making reproducible, dynamic reports. -->
<!-- It's kind of like Microsoft Word: it publishes documents. -->
<!-- But in contrast, you can use Quarto to make reports with markdown, but also so many other elements, like narrative text, code chunks (several languages, like R, Python... Latex...) -->
<!-- (keep going through bullets) -->
...
Once your acronym is in the glossary, find your acronym in your report and encase it in curly brackets ({}), with "\gls" preceding it. For example, to indicate "NOAA" is an acronym, you would write `\gls{noaa}`. Use this notation each time you use the acronym in your text.

Your text will look like this:

> This is the first instance of `\gls{noaa}`. Here, `\gls{NOAA}` is used a second time.

And it will render like this:

> This is the first instance of National Oceanic and Atmospheric Administration (NOAA). Here, NOAA is used a second time.
...
<!-- A few tips for working with Quarto docs, which we've learned over our time transitioning from R markdown to Quarto -->
...
<!-- Ask who knows what abs, rel pathnames are -->
<!-- give example with AFSC address for abs, rel pathnames -->
