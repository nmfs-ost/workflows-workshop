...
<!-- Before asar/stockplotr: cover a few topics that build the foundation of these tools -->
<!-- This info will be publicly available, so you can always refer back to it -->
...
<!-- With markdown, you use symbols to signify when text is bolded, italicized, are in a list, etc. -->
<!-- I'll just name some examples below- you can always reference this page (publicly available), and check out the asar vignette with way more info about markdown formatting options. -->
<!-- Pound signs signify header levels -->
<!-- Asterisks can make text italic or bold -->
<!-- Create unordered lists with hyphens and asterisks; ordered lists with numbers -->
<!-- Add links with square brackets and parentheses. -->
<!-- Can add math to your documents- in a paragraph, and in its own paragraph. LaTeX-style formatting is acceptable in markdown files. -->
...
<!-- *Who here is familiar with Quarto?* -->
<!-- If you're familiar with it, you can think of it like a next-generation R markdown; like it, but with more features and flexibility. -->
<!-- Why use quarto? -->
<!-- -Create dynamic reports combining markdown, code (like R, Python, LaTeX, and more), the outputs of that code (like tables and figures), and all kinds of elements into one document -->
<!-- -Reproducible: others can replicate your work in a straightforward way -->
<!-- -You can produce reports in several formats like HTML, PDF, and Word -->
<!-- -Quarto will *save time and reduces errors*. If your data changes, you don't have to manually rerun your analysis and copy/paste the results. You just re-knit the document, and everything is updated automatically. -->
<!-- You'll be writing your asar-based report with Quarto. Most files are quarto files: qmd extension. -->
<!-- Actually, the page you're reading was rendered from a Quarto document! Shows some of the many features available, like callouts, headers, links, etc. -->
...
<!-- Parameters are objects you can reference in a report. Example: "species" = "Petrale sole" -->
<!-- In report, you'd basically write "species" (with a specific syntax) and, when your report renders, "species" would change to "Petrale sole". -->
<!-- The syntax: 'params' + dollar sign + param name -->
<!-- They're great bc if you want to change the species- e.g., if your next report is about salmon- you simply change "species" in the YAML once, instead of dozens of times throughout report. -->
<!-- -esp. great for params like "year" -->
...
<!-- Add citations -->
<!-- -Check out the .bib file that is produced when you create an asar report- contains hundreds of citations -->
<!-- --you can also add to it; reference managers export citations to .bib format, as do journal websites -->
<!-- In the bib file, you'll see a "citation key"- the name of the citation, before its title, volume, etc. Just add that to your report, with an @ sign in front, to add a citation. -->
<!-- Great for if you need to edit all citations- example: you realized year was incorrect. Just edit entry in bib file and recompile. No manual editing each citation! -->
<!-- You'll also see a list of the references cited in your report in the "References" section. -->
...
<!-- When you create an asar report, you'll see that the report is set up in a *modular* fashion. This means that, instead of having one giant file with your entire report's text in it, your report will be assembled by pulling in text from several different files. -->
<!-- You'll have two main groups of files: -->
<!-- -child docs (one file for the exec summary, one file for the introduction, etc.) -->
<!-- -skeleton (the file that pulls in the child docs, and which is rendered into the report) -->
<!-- This setup helps with collaboration because people aren't all working on the same file at the same time. Helps reduce conflicts when everyone's changes are submitted. -->
...
<!-- Last general topic: absolute vs relative pathnames -->
<!-- *Who's familiar with these terms?* -->
<!-- An absolute filepath: entire path from your computer's root (like, C drive) to the document. -->
<!-- Relative filepath: a portion of the absolute filepath- starting at a different folder. -->
<!-- Example: Absolute filepath is the entire address of this building. Relative filepath: Just the street address and town- no zip code, state, or country. It's the *local* address. -->
<!-- With Quarto, if you, e.g., import a file into a quarto doc, you have to use either an absolute filepath, OR a relative filepath that is created in relation to the Quarto doc itself. -->
<!-- Example: (review ex w/fish files) -->
<!-- Not enough time to cover code chunks today- feel free to explore yourself. -->
<!-- Now we'll take a break until 10am. Feel free to use the time for anything you need. Take a break; try some practice questions, with answers; ask us questions! We'll start at 10am with an intro to asar. -->
...
![asar yaml controlling formatting and basic document information.](/Curriculum/Day_2/images/asar_yaml.png){fig-alt="A screenshot of an example yaml, with curly brackets highlighting which code is 'information' and which code is 'formatting'."}
...
<!-- Now that you've generated your report outline, let's make figures and tables to add to it! -->
<!-- We'll use the R package `stockplotr`. -->
<!-- -Our goal: create a centralized package that contains all of the figures and tables for a stock assessment report, which are generalized and not specific to any one region. -->
<!-- -Saw a need for one centralized pkg since multiple packages that make figs/table but are typically region and/or model-dependent -->

## Package information

<!-- Here is a table showing the available figures and tables you can make, with their associated function (we're working on tables besides landings rn) -->
<!-- Functions return a `ggplot2` object (figure) or `gt` object (table) -->
<!-- Then, you can further customize them using notation you are familiar with: -->
<!-- Figures: can add onto the plot with other `ggplot2` functions or extensions using the plus (+) sign -->
<!-- Tables: use the native pipe (|>) when adding additional formatting, rows, etc -->
...
<!-- Using the 'petrale' object, which is the model results file you converted just a few minutes ago: -->
...
<!-- Now let's customize even more -->
<!-- All figures automatically identify any indexing variables in the data (e.g., fleet, area, or sex) -->
<!-- You can use the `group` and `facet` arguments to modify how these indexing variables are displayed in the figure -->
<!-- You can also modify other arguments such as `geom`, `ref_line`, and `relative` to change the appearance of the figure.  -->
...
<!-- This example: -->
<!-- -we're setting the reference line to 260 -->
<!-- -we're specifying the module name to "time series" so that we won't have to answer the question in the console each time -->
...
<!-- This example: -->
<!-- building on the prev customization, AND: -->
<!-- -making the plot show *relative* biomass (biomass relative to the ref line) AND changing linewidth to be bolder (with a classic `ggplot2` command) -->
...
<!-- We don't have time for this right now, but you can see the tip below to learn how to remove a ref line and customize your figures even more. -->
<!-- Questions? -->
...
<!-- -exported as `gt` objects; can be edited/customized as such (use the native pipe) -->
...
<!-- There are many more ways to customize tables; click the link in the tip box if you'd like to learn more! -->
<!-- Questions? -->
...
<!-- Export plots into rdas: R data files (like a list of objects) -->
<!-- Figures: figure, caption, alt text -->
<!-- Tables: `gt`-based table, caption, table in latex format -->
<!-- *Run example and explore associated rda* -->
...
<!-- Can click link in the tip box to learn more about table rdas, and how to automatically create and export all figures/tables -->
<!-- Questions? -->
...
s
<!-- We just learned how to make figures and tables with `stockplotr`. Now: add plots into `asar` reports -->
<!-- Remember: running `create_template()` creates 'report' folder w/many files, incl. docs that will contain code to display figures and tables (like '08_tables.qmd' and '09_figures.qmd') -->
<!-- Currently, those files are empty. Let's add figs/tables now! -->
...
<!-- Two ways to do this. We'll show first way today (rerunning `create_tables/figures_doc()`) and give you info to explore second way (manually adding them) -->
...
<!-- A major component of accessible doc: tags -->
<!-- Tags: structural components of PDFs, help screen readers navigate doc -->
<!-- -ID headers, paragraph text, images, etc. -->
<!-- We use add_tagging() to add tags to the latex version of our qmd report: named something like 'SAR_species_skeleton.tex' (note extension) -->
<!-- -doing this bc Quarto doesn't offer tagging yet (v1.9 hopefully!) -->
<!-- See code to add tagging, below. Note we use `withr::with_dir()` function as a wrapper for `add_tagging()` b/c it sets the "report" folder as a temporary wd -->
<!-- -Could remove wrapper if you set the "report" folder as the wd -->
<!-- Note the arguments (explain) -->
...
<!-- Next step: add alt text -->
<!-- -anyone know what this is? (then, explain- telephone example) -->
<!-- Our workflow involves adding AT to a spreadsheet, then adding it into the same .tex file -->
<!-- -using stockplotr to make figs sets up the spreadsheet -->
...
![2. In the 'Prepare for accessibility', click the "Add alternative text" button.](Day_3/images/Check_alttext-2.png){fig-alt="Screenshot of a document opened in Adobe. The 'Prepare for accessibility' menu shows a red oval around the 'Add alternative text' button."}

![3. In the 'Set alternative text' pop-up, verify that your images contain the expected alternative text and edit as necessary.](Day_3/images/Check_alttext-3.png){fig-alt="Screenshot of a document opened in Adobe. The 'Set alternative text' pop-up shows where an example image's alternative text is located."}

<!-- We've reached the end! -->
<!-- Can use remaining time to practice, ask questions, etc -->
<!-- We also ask that you tell us what you thought about today's workshop  by answering 3 quick questions: What went well? What could be improved? What is a question you still have? -->
<!-- This will help us improve the workshop, which we'll be presenting in a longer form several times this winter/spring. -->
<!-- Thank you for your attention and effort! Again, can use remaining time to practice, ask questions, etc -->
