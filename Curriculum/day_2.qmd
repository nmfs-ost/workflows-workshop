...
they want. The function contains a set of defaults that would produce a tempalte that could be rendered,
but contains no real information relevant to the target stock.

So, let's do more than just create a blank template! The following arguments are some key parts to fill
for a specific stock assessment report:
