...
`asar` revolves around a few primary functions*, but contains a variety of support functions that help make the template. While most of them are accessible to the user, they would not perform helpful functions for them. We will be going over the following functions:

- `create_template()`
- `convert_output()`* (*This function is now located within `stockplotr` but is useful for building `asar` reports)
- `create_tables_doc()`
- `create_figures_doc()`
