...
Welcome! Please open the [Day 2 communal notes doc](https://docs.google.com/document/d/1lcBCPdqQSmKcxR5O1Tv7Rv6n1UdZWsCxY7Xk28AJLXc/edit?tab=t.ds1fl2nu2oi5#heading=h.lwix6dbx3lr8) and participate in the icebreaker exercise.
...
-   [Day 2 communal notes doc](https://docs.google.com/document/d/1lcBCPdqQSmKcxR5O1Tv7Rv6n1UdZWsCxY7Xk28AJLXc/edit?tab=t.ds1fl2nu2oi5#heading=h.lwix6dbx3lr8)
...
create_template(
  ...
  authors = c("Samantha Schiano" = "OST", "Jane Doe"="PIFSC"),
  file_dir = getwd() # folder where the "report" folder will be saved
)
```

::: {.callout-tip}
Notice the `authors` syntax. It contains a vector with:

1. The author's first and last name
2. `=` sign
3. Initials of the author's office or region (their affiliation)
* To denote a specific location within a region, add a hyphen followed by the first letter of each word of the location (city, state). Example: "SWFSC-LJCA" stands for "Southwest Fisheries Science Center, La Jolla, California; "AFSC-NOR" stands for "Alaska Fisheries Science Center, Newport, Oregon".

All affiliation acronyms are in the ["affiliations" file](https://github.com/nmfs-ost/asar/blob/main/inst/resources/affiliation_info.csv).
:::
...
::: {.callout-note}
Your skeleton file will *not* be called "skeleton.qmd". Its filename will be comprised based on the arguments you added to `create_template()`, such as the report type, region, and species.

For the example above, it will be called "sar_USWC_Petrale_sole_skeleton.qmd".
:::
...
# convert the output
paka <- stockplotr::convert_output(
  output_file,
  save_dir = file.path(getwd(), "report", "paka_conout.rda")
  )

# copy the converted output to your wd too, for later usage
file.copy(
  from = file.path(getwd(), "report", "paka_conout.rda"),
  to = getwd()
  )
```
...
::: {.callout-important}
You should provide the `model_results` argument with the path to your model results file *relative to your report skeleton*.
For example, if your model results file is in the same folder as your skeleton, you would just provide the filename.
If it's the parent folder of "report", you would provide "../filename".
If it's in a subfolder of "report", you would provide "subfolder/filename".
:::
...
asar::create_template(
  rerender_skeleton = TRUE,
  file_dir = file.path(getwd(), "report"),
  model_results = file.path("paka_conout.rda"), # add the path to your model results file *in relation to your report skeleton*
  authors = c("Michelle Sculley"="PIFSC")
)
```
...
**We are creating a new report, so please set your working directory to a new folder or rename our previous "main" report folder to "main_report" or something along those lines.**

```{r eval=FALSE}
asar::create_template(
  # Previous arguments we used for main report
  type = "sar",
  office = "PIFSC",
  region = "Hawaii",
  species = "Crimson jobfish",
...
**Now, let's rename our folders: "report" --> "custom_report", and "main_report" --> "report". This will allow us to continue working with our original report tomorrow by adding figures and tables.**
...
<!-- D identifies mistake in understanding purpose of create_template(). create_template() is a tool that builds the structure of your report, but not the detailed content of it (body of text). -->
